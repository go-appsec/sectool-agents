...
- **`controller.py`** — main orchestrator loop (`asyncio`). Implements the same three-phase iteration as secagent: per-worker autonomous run → verification (multi-substep, capped at `VERIFICATION_MAX_SUBSTEPS=6`) → direction (multi-substep, capped at `DIRECTION_MAX_SUBSTEPS=4`, plus one mandatory self-review substep). Owns MCP bring-up/attach, signal handling, and the verifier/director phase drivers + prompt formatting. Every billed API submission must go through `submit_query(client, prompt)` (defined in `runtime.py`) — never call `client.query` directly, or spacebar pause will leak (a unit test in `tests/test_pause.py` enforces this; the audit scans `controller.py`, `worker.py`, and `runtime.py`).
- **`worker.py`** — SDK-client lifecycle and worker subsystem. Defines `ManagedSDKClient` (runs each `ClaudeSDKClient` in its own asyncio task to isolate the SDK's anyio cancel scopes), `WorkerState`, `collect_worker_turn`, the autonomous loop (`run_worker_until_escalation`, `run_all_workers_until_escalation`), recovery helpers (`attempt_worker_recovery`, `attempt_client_recovery`, `reset_orchestrator_client`), and `synthesize_and_teardown_recon`. Has one lazy import of `_build_worker_continue_prompt` from `controller.py` to avoid a module cycle.
- **`runtime.py`** — shared runtime primitives: `log`, the `_pause_gate` event + `submit_query` chokepoint, `engage_rate_limit_pause`, `toggle_pause`, the `InflightRegistry`, the `StatusBar` ANSI scroll-region status line, and `_clear_leaked_cancellations`. Both `controller.py` and `worker.py` import from here; nothing in `runtime.py` imports from the project.
